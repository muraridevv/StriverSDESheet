...
public class JsonPathParser {
  ...
  public static void main(String[] args) {
    String jsonPath1 =
        "$.regulator_cap_override[?(@.regulatory_cap_rate=='001' && @.industry_segment_code == '22')]";
    String jsonPath2 =
        "$.interchange_local_pricing.regulator_cap_override[?(@.regulatory_cap_rate=='001')]";
    String jsonPath3 =
        "$.speed_of_pay_data[?(@.transaction_class_code == 'A')].interchange_regional_pricing[?(@.pricing_region_identifier == '051')].regulatory_cap_override[?(@.regulatory_cap_rate == 'B')]";
    String jsonPath4 = "$";
    try {
  ...
  private static void printJsonFromPath(String jsonPath) {
    try {
      ...
      throw new IllegalArgumentException(
          "Unexpected error while processing JSON Path: " + jsonPath, e);
    }
  }
  ...
  public static String[] parseJsonPath(String jsonPath) {
    try {
      ...
      Pattern pattern =
          Pattern.compile(
              "[^.\\[]+(\\[\\?\\(.*?\\)\\])?"); // Match keys with optional array filters
