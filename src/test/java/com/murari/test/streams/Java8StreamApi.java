...
public class Java8StreamApi {
    ...
    public void checkOrder_whenChangeQuantityOfMethodCalls_thenCorrect(){
        ...
        counter = 0;
        List<String> listStream1 = list.stream().skip(2).map(element -> {
            counter++;
        ...
        counter=0;
        List<String> listStream2= list.stream().map(element->{
            counter++;
            return element.substring(0,3);
        }).skip(2).toList();
    ...
    public void runStreamPipeline_whenOrderIsRight_thenCorrect(){
        List<String> list= Arrays.asList("abc1", "abc2", "abc3");
        Optional<String> optionalStream= list.stream().filter(element->{
    ...
    public void reduce_whenExpected_thenCorrect(){
        ...
        int reduceThreeParamsParallel= Arrays.asList(1,2,3).parallelStream().reduce(10, (a,b)-> a+b,(a,b)->{
            System.out.println("combiner was called");
            return a+b;
        });
    ...
    public void collecting_whenAsExpected_thenCorrect(){
        ...
        Assertions.assertTrue(listToString.contains(",") && listToString.contains("[") && listToString.contains("]"));
        ...
        Map<Integer, List<Product>> collectorMapOfList= productList.stream().collect(Collectors.groupingBy(Product::getPrice));
        Assertions.assertEquals(3, collectorMapOfList.size());

        Map<Boolean, List<Product>> mapPartitioned= productList.stream().collect(Collectors.partitioningBy(element-> element.getPrice()>15));
        Assertions.assertEquals(2,mapPartitioned.keySet().size());
    }

    @Test
    public void collect_whenThrows_thenCorrect(){
        Set<Product> unModifiedSet= productList.stream().collect(Collectors.collectingAndThen(Collectors.toSet(), Collections::unmodifiableSet));
