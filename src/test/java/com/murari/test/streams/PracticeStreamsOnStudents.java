...
public class PracticeStreamsOnStudents {
    ...
    public void check_firstNameStartsWithA_thenCorrect(){
        List<Student> list= studentList.stream().filter(student -> student.getFirstName().startsWith("A")).toList();
    ...
    public void check_groupByDepartment_thenCorrect(){
        Map<String, List<Student>> departmentmap= studentList.stream().collect(Collectors.groupingBy(Student::getDepartmentName));
    ...
    public void check_maxAgeOfStudent_thenCorrect(){
        IntSummaryStatistics statistics= studentList.stream().collect(Collectors.summarizingInt(Student::getAge));
    ...
    public void check_departmentName_thenCorrect(){
        List<String> departmentNames= studentList.stream().map(Student::getDepartmentName).distinct().toList();
    ...
    public void check_countOfStudentInEachDepartment_thenCorrect(){
        Map<String,Long> studentCount= studentList.stream().collect(Collectors.groupingBy(Student::getDepartmentName,Collectors.counting()));
    ...
    public void check_rankBetweenGivenNumber_thenCorrect(){
        long studentCount= studentList.stream().filter(student -> student.getRank()>50 && student.getRank()<100).count();
    ...
    public void check_averageAgeOfMaleAndFemale(){
        Map<String, Double> averageAge= studentList.stream().collect(Collectors.groupingBy(Student::getGender,Collectors.averagingDouble(Student::getAge)));
    ...
    public void check_departmentHavingMaximumStudent(){
        Map<String,Long> deparmentMap = studentList.stream().collect(Collectors.groupingBy(Student::getDepartmentName,Collectors.counting()));
        Optional<Map.Entry<String, Long>> maxDepartment = deparmentMap.entrySet().stream()
    ...
    public void check_secondRank_thenCorrect(){
        Optional<Student> firstByRank = studentList.stream()
                .sorted(Comparator.comparing(Student::getRank))
                .skip(1)
                .findFirst();
    ...
    public void checkIfAnyStudentHasHighRank_thenCorrect() {
        boolean anyStudentWithHighRank= studentList.stream()
                .anyMatch(student -> student.getRank()>300);
...
//     19. find students whose first name starts with the letter 'A'.
    ...
    public void findStudentsWithFirstNameStartingWithLetter_thenCorrect(){
            long  studentsNameStartsWithA= studentList.stream()
    ...
    public void calculateAgeSquared_thenCorrect() {
        List<Student> stuentsAgeSquaredList= studentList.stream()
    ...
    //    calculate the weighted average of rank based on student age, where older students' ranks contribute more weight.
    @Test
    public void calculateWeightedAverageRankByAge_thenCorrect() {
        double weightedAverageRank= ((studentList.stream()
                .mapToDouble(student-> student.getAge()* student.getRank())
                .sum() )
                /
                (studentList.stream()
